...
def test():
    ...
    samples_ref_base = np.frombuffer(content, dtype = 'uint16').reshape((-1,256,256))
    ...
    net.load_state_dict(mge.load("./model/test_集成_model"))
    net.eval()
    ...
    import tqdm
    print("len(samples_ref)=",len(samples_ref_base))
    batchsz = 8

    for k in range(4):
        if(k == 0):
            fout = open('workspace/集成_1.bin', 'wb')
            samples_ref = samples_ref_base
        elif(k == 1):
            fout = open('workspace/集成_2.bin', 'wb')
            samples_ref = samples_ref_base[:, :, ::-1]
        elif (k == 2):
            fout = open('workspace/集成_3.bin', 'wb')
            samples_ref = samples_ref_base[:, ::-1, :]
        elif (k == 3):
            fout = open('workspace/集成_4.bin', 'wb')
            samples_ref = samples_ref_base[:, :, ::-1]
            samples_ref = samples_ref[:, ::-1, :]
